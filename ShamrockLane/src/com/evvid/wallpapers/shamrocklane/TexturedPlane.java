...
public class TexturedPlane extends BaseObject3D {
	protected float mWidth;
	protected float mHeight;
	protected float[] mCustomTexCoords;
	...
	public TexturedPlane(float width, float height, int segmentsW, int segmentsH) {
		this(width, height, 1, segmentsW, segmentsH);
	}
	
	public TexturedPlane(float width, float height, int direction, int segmentsW, int segmentsH, float[] customCoords) {
		super();
		mWidth = width;
		mHeight = height;
		...
		mCustomTexCoords = customCoords;
		init();
	}
	
	public TexturedPlane(float width, float height, int direction, int segmentsW, int segmentsH, boolean rotatedTexCoords) {
		this(width, height, direction, segmentsW, segmentsH, null);
		mRotatedTexCoords = rotatedTexCoords;
	}
	
	public TexturedPlane(float width, float height, int direction, int segmentsW, int segmentsH) {
	...
	private void init() {
		...
		if(mCustomTexCoords != null)
			textureCoords = mCustomTexCoords;
